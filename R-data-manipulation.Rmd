...
Slides: https://rpubs.com/bpbond/1392825
...
Let's say we want to put `people` into long--or _tidy_--format, in which 
every row is a different observation. (Wide-form data tend to be convenient 
for _people_ to read, while long-form data are better for data processing.)
...
Note the message. It's good practice to specify the **common columns** and
the **relationship** that joins should use:

```{r, eval=FALSE}
people %>% left_join(hometowns, by = "who", relationship = "one-to-one")
```
...
This former code now generates a warning. I've left it here in the slides,
...
- Efficient and high performance computing
